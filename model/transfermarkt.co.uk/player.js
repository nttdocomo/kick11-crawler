...
Player.get_player_by_team = function($,team_id){
  ...
  first_season = $("select[name='saison_id']").find("option:first-child").text(),
  competition_code = $("select[name='wettbewerb_select_breadcrumb']").find("option:selected").val(),
  playerCells = $('.hide-for-small > a.spielprofil_tooltip'),
  players = [],
  events,
  season_id,
  team_id,
  ...
  console.log(season == first_season)
  if(season == first_season){
    return excute(mysql.format('SELECT team_id FROM `transfermarkt_team_team` WHERE transfermarkt_team_id = ? LIMIT 1',[team_id])).then(function(row){
      ...
      return players.reduce(function(sequence, player){
        return sequence.then(function(){
          ...
          return excute(mysql.format('SELECT id,team_id FROM `teamplayer` WHERE event_id IS NULL AND player_id = ? LIMIT 1',[player_id]))
        }).then(function(row){
          if(row.length){
            if(row[0].team_id != team_id){
