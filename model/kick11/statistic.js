...
Model.getMatchCentrePlayerStatistics = function(whoscored_match_player_statistic){
    ...
    return excute(mysql.format('SELECT team_id FROM `whoscored_team_team` WHERE whoscored_team_id = ? LIMIT 1',[whoscored_match_player_statistic.teamId])).then(function(row){
        ...
        return excute(mysql.format('SELECT match_id FROM `whoscored_match_match` WHERE whoscored_match_id = ? LIMIT 1',[whoscored_match_player_statistic.matchId])).then(function(row){
            if(!row.length){
                return Promise.resolve()
            }
            match_id = whoscored_match_player_statistic.matchId = row[0].match_id;
            return excute(mysql.format('SELECT player_id FROM `whoscored_player_player` WHERE whoscored_player_id = ? LIMIT 1',[whoscored_match_player_statistic.playerId])).then(function(row){
                if(!row.length){
                    return Promise.resolve()
                }
                player_id = whoscored_match_player_statistic.playerId = row[0].player_id;
                return excute(mysql.format('SELECT id FROM `match_player_statistics` WHERE playerId = ? AND teamId = ? AND matchId = ? LIMIT 1',[player_id,team_id,match_id])).then(function(row){
