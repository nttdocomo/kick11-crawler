...
excute('SELECT id FROM `event`').then(function(events){
	return events.reduce(function(sequence,event){
		return sequence.then(function(){//查看event_standings里有没有对应的event_id的记录
			return excute(mysql.format('SELECT 1 FROM `event_standings` WHERE event_id = ? LIMIT 1',[event.id]));
		}).then(function(row){
			if(!row.length){
			...
			return excute('SELECT team_id FROM `event_team` WHERE event_id = ?',[event.id]).then(function(event_team){
				...
				return event_team.reduce(function(sequence,item){
					...
					return excute(mysql.format('SELECT * FROM `match` WHERE round_id IN (SELECT id FROM `round` WHERE event_id = ?) AND team1_id = ? OR team2_id = ? AND score1 IS NOT NULL AND score2 IS NOT NULL',[event.id,item.team_id,item.team_id])).then(function(matches){
						...
						return event_standing_entries.reduce(function(sequence,event_standing_entry){
							return sequence.then(function(){
								return excute(mysql.format('SELECT 1 FROM `event_standing_entries` WHERE event_id = ? AND team_id = ? LIMIT 1',[event_standing_entry.event_id,event_standing_entry.team_id]))
							}).then(function(row){
								if(!row.length){
									return excute(mysql.format('INSERT INTO `event_standing_entries` SET ?',event_standing_entry))
								} else {
									return excute(mysql.format('UPDATE `event_standing_entries` SET ? WHERE event_id = ? AND team_id = ?',[event_standing_entry,event_standing_entry.event_id,event_standing_entry.team_id]))
								}
							})
						},Promise.resolve())
					})
