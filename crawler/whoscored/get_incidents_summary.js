        ...
        if(/^var\S+?(\[\[\[.+?\])\;\S+?\)\;$/.test(html)){
            ...
            events.forEach(function(event){
                ...
                if(event_name == 'goal'){
                    ...
                    excute(mysql.format('INSERT INTO whoscored_goals (player_id,match_id,team_id,minute,score1,score2,created_at) SELECT ? FROM dual WHERE NOT EXISTS(SELECT id FROM whoscored_goals WHERE player_id = ? AND match_id = ? AND team_id = ? AND minute = ?)', [[player_id,match_id,team_id,minute,score1,score2,moment.utc().format('YYYY-MM-DD hh:mm:ss')],player_id,match_id,team_id,minute]));
                }
            })
        }
    })
