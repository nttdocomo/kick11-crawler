...
module.exports = function(content){
	return Promise.resolve().then(function(){
    	...
    	return excute(mysql.format('SELECT id FROM `transfermarkt_competition` WHERE code = ?',[transfermarkt_competition_code])).then(function(row){
    		if(!row.length){
    		...
    		return excute(mysql.format('SELECT id FROM `transfermarkt_season` WHERE title = ?',[title])).then(function(){
    			if(!row.length){
    		...
    		return excute(mysql.format('SELECT id FROM transfermarkt_event WHERE competition_id = ? AND season_id = ? LIMIT 1',[transfermarkt_competition_id,transfermarkt_season_id])).then(function(){
    			if(!row.length){
	    			...
	    			return excute(mysql.format('SELECT * FROM `transfermarkt_event_event` WHERE transfermarkt_event_id = ?',[row[0].id])).then(function(row){
	    				transfermarkt_event_id = row[0].transfermarkt_event_id;
			...
			return matchdays.reduce(function(sequence,el,index){
				...
				return sequence.then(function(){
					return excute(mysql.format('SELECT id FROM `transfermarkt_round` WHERE transfermarkt_event_id = ? AND round = ? LIMIT 1',[transfermarkt_event_id,pos])).then(function(round){
						if(round.length){
